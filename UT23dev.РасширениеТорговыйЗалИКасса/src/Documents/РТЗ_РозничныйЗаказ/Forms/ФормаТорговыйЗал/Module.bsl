...
Процедура ПриОткрытии(Отказ)
	...
	Попытка
		...
		ЗаполнитьПараметрыТерминалаРозничныйИнтерфейс();
		
	Исключение
		
		ОбщегоНазначенияКлиент
			.СообщитьПользователю("Ошибка при заполнении настроек, обратитесь в IT-отдел!",,, Объект);
		Отказ = Истина;
...
// Заполнить параметры терминала розничный интерфейс.
&НаСервере
Процедура ЗаполнитьПараметрыТерминалаРозничныйИнтерфейс()
	...
	Или  Не ЗначениеЗаполнено(ЦенаВключаетНДС) Тогда
		ВызватьИсключение "Не коректное заполнение параметров";
	КонецЕсли;
...
&НаСервере
Функция СоздатьНовыйЗаказ(Менеджер, ВидЦены, Дата = Неопределено)
	
	ДокументЗаказа = Документы.РТЗ_РозничныйЗаказ.СоздатьДокумент();
	
	ДокументЗаказа.Дата 	= Дата; 
...
&НаКлиенте
Процедура ПослеЗакрытияВводаПароля(Менеджер, ДопПараметры = Неопределено) Экспорт
	
	Если Не ЗначениеЗаполнено(Менеджер) Тогда
		Возврат;
	Иначе
		Заказ = СоздатьНовыйЗаказ(Менеджер, ВидЦены, ОбщегоНазначенияКлиент.ДатаСеанса());
		РТЗ_ПечатьЗаказаКлиент.ВывестиПользователюЗаказКлиент(Заказ, Неопределено);
	КонецЕсли;
